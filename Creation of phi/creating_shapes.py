...
import time
import numpy as np 
from vedo.dolfin import plot, screenshot, closePlotter, interactive 
# dolfin parameters
...
start,end,step = 1,8,1

# Computation of the standard FEM     
for i in range(start, end, step):
    H = 5*2**i # to have approximately the same precision as in the phi-fem computation
    domain = mshr.Ellipse(df.Point(0.0,0.0), 1.0,0.5) + mshr.Circle(df.Point(-1.0,0.375), 0.25) + mshr.Rectangle(df.Point(-0.6,0.0), df.Point(-0.4,-1.0)) \
       + mshr.Rectangle(df.Point(0.6,0.0),df.Point(0.4,-1.0)) - mshr.Circle(df.Point(-1.15,0.45), 0.05, 10)
    mesh = mshr.generate_mesh(domain, 2*H)
...
for i in range(start,end,step): 
    ...
    H = 5*2**i
    domain = mshr.Ellipse(df.Point(0.0,0.0), 1.0,0.5) + mshr.Circle(df.Point(-1.0,0.375), 0.25) \
           + mshr.Rectangle(df.Point(-0.6,0.0), df.Point(-0.4,-1.0)) \
           + mshr.Rectangle(df.Point(0.6,0.0),df.Point(0.4,-1.0)) - mshr.Circle(df.Point(-1.15,0.45), 0.05, 10)
    init_mesh = mshr.generate_mesh(domain, 2*H)
    boundary_mesh = df.BoundaryMesh(init_mesh, "exterior", True)
    points = boundary_mesh.coordinates()
    boundary_mesh_tree = boundary_mesh.bounding_box_tree()
    tree = init_mesh.bounding_box_tree()
    nbr_cells = int(df.sqrt(32)/hh_standard[i-1])
    nbr_points.append(len(points))
    square = df.RectangleMesh(df.Point(-2,-2), df.Point(2,2), nbr_cells,  nbr_cells) # modifier H pour avoir hmax = hmax du cercle
    ...
    #interactive()
    screenshot('creating_shapes/shape'+str(i)+'.png')
    closePlotter()  
...
plt.figure())
plt.loglog(hh_phi,nbr_points,'-o')
plt.xlabel('Size of elements (h)')
plt.ylabel("Number of points")
plt.title(r'Number of points on the initial scatter plot  ')
plt.savefig('nbr_points_P_{name0}.png'.format(name0=degV))
plt.show()
