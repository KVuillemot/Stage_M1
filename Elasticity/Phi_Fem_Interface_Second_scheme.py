...
for i in range(start,end,step):
    ...
    W = mph.BlockFunctionSpace([V,V,Z,Z,Q,Q], restrict=[Omega1, Omega2, Interface, Interface, Interface, Interface])
    uypP = mph.BlockTrialFunction(W)
    (u1, u2, y1, y2, p, P) = mph.block_split(uypP)
    vzqQ = mph.BlockTestFunction(W)
    (v1, v2, z1, z2, q, Q) = mph.block_split(vzqQ)
    ...
    u_ex = df.Expression(('cos(x[0])*exp(x[1])', 'sin(x[1])*exp(x[0])'), 
    ...
    ay1Q  = gamma_y*h**(-3)*df.inner(df.dot(y1, df.grad(phi)),Q*phi)*dx(interf)
    ...
    ay2Q  = -gamma_y*h**(-3)*df.inner(df.dot(y2, df.grad(phi)),Q*phi)*dx(interf)
    ...
    aPz1 = gamma_y*h**(-3)*df.inner(P*phi,(df.dot(z1,df.grad(phi))))*dx(interf)
    aPz2 = -gamma_y*h**(-3)*df.inner(P*phi,(df.dot(z2,df.grad(phi))))*dx(interf)
    aPQ  = gamma_y*h**(-4)*df.inner(P*phi,Q*phi)*dx(interf)
    ...
    A = [[au1v1, au1v2, au1z1, 0.0, au1q, 0.0], 
            [au2v1, au2v2, 0.0, au2z2, au2q, 0.0],
            [ay1v1, 0.0, ay1z1, ay1z2, 0.0, ay1Q],
            [0.0, ay2v2, ay2z1, ay2z2, 0.0, ay2Q],
            [apv1,  apv2, 0.0, 0.0,  apq,  0.0],
            [0.0, 0.0,  aPz1,  aPz2,  0.0,  aPQ]]
    L = [lv1, lv2, lz1, lz2, 0.0, 0.0]
...
if E1 == E2 and nu1 == nu2 :
    f = open('second_scheme_elasticity_same_parameters_P{name0}.txt'.format(name0=degV),'w')
else :
    f = open('second_scheme_elasticity_different_parameters_P{name0}.txt'.format(name0=degV),'w')
