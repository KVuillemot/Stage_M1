...
for i in range(start,end,step): 
    ...
    W = mph.BlockFunctionSpace([V,Z,Q_N,Q_D], restrict=[None,yp_res,yp_res,yp_res])
    uyp = mph.BlockTrialFunction(W)
    (u, y, p_N, p_D) = mph.block_split(uyp)
    vzq = mph.BlockTestFunction(W)
    (v, z, q_N, q_D) = mph.block_split(vzq)
    
    gamma_div, gamma_u, gamma_p, sigma_p, gamma_D, sigma_D = 1.0, 1.0, 1.0, 0.01, 20.0, 1.0
    ...
    boundary_penalty = sigma_p*df.avg(h)*df.inner(df.jump(sigma(u),n), df.jump(sigma(v),n))*dS(Neumann) \
                     + sigma_D*df.avg(h)*df.inner(df.jump(sigma(u),n), df.jump(sigma(v),n))*dS(Dirichlet) \
    ...
    auv = df.inner(sigma(u), epsilon(v))*dx  \
        ...
        - df.inner(df.dot(sigma(u),n),v)*ds(Dirichlet) 
...
for i in range(start, end, step):
    ...
    error_l2_standard.append(df.sqrt(df.assemble((df.inner(u_ex-u,u_ex-u)*df.dx)))/df.sqrt(df.assemble((df.inner(u_ex,u_ex))*df.dx)))            
    error_h1_standard.append(df.sqrt(df.assemble((df.inner(df.grad(u_ex-u),df.grad(u_ex-u))*df.dx)))/df.sqrt(df.assemble((df.inner(df.grad(u_ex),df.grad(u_ex)))*df.dx))) 
...
f = open('output_mixed_P{name0}.txt'.format(name0=degV),'w')
