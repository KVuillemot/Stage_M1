...
class phi1_expr(df.UserExpression) : 
    def eval(self, value, x):
        value[0] = -0.15**2 + (x[0]-0.25)**2 + (x[1]-0.25)**2 
...
for i in range(start,end,step): 
    ...
    f = df.Expression(('0.0','-9.81*x[1]'), degree= 4, domain = mesh)
    ...
    f = df.Expression(('0.0','-9.81*0.01 '), degree=4, domain = mesh)
    g = df.Constant((0.0,0.0))
    ...
    domain = mshr.Rectangle(df.Point(0,0), df.Point(1,1)) \
        - mshr.Circle(df.Point(0.25,0.25), 0.15) - mshr.Circle(df.Point(0.75,0.75), 0.15)
    H = 8*2**(3) # to have approximately the same precision as in the phi-fem computation
