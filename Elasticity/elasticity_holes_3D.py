...
def sigma(u):
    return lambda_ * df.div(u)*df.Identity(3) + 2.0*mu*epsilon(u)
...
class phi1_expr(df.UserExpression) : 
    def eval(self, value, x):
        value[0] = -0.15**2 + (x[0]-0.25)**2 + (x[1]-0.25)**2 + (x[2]-0.25)**2 

    def value_shape(self):
        return (2,)
class phi2_expr(df.UserExpression) : 
    def eval(self, value, x):
        value[0] = -0.15**2 + (x[0]-0.75)**2 + (x[1]-0.75)**2 + (x[2]-0.75)**2 
...
for i in range(start,end,step): 
    ...
    for cell in df.cells(square):  
        v1,v2,v3,v4 = df.vertices(cell)
        if(phi1(v1.point()) >= 0.0 or phi1(v2.point()) >= 0.0 or phi1(v3.point()) >= 0.0 or phi1(v4.point()) >= 0.0 or df.near(phi1(v1.point()),0.0) or df.near(phi1(v2.point()),0.0) or df.near(phi1(v3.point()),0.0) or df.near(phi1(v4.point()),0.0)):
            if(phi2(v1.point()) >= 0.0 or phi2(v2.point()) >= 0.0 or phi2(v3.point()) >= 0.0 or phi2(v4.point()) >= 0.0 or df.near(phi2(v1.point()),0.0) or df.near(phi2(v2.point()),0.0) or df.near(phi2(v3.point()),0.0) or df.near(phi2(v3.point()),0.0)):
                Cell_omega[cell] = 1
    ...
    edge_sub = df.MeshFunction("bool", mesh, mesh.topology().dim()-2)    
    vertices_sub = df.MeshFunction("bool", mesh, mesh.topology().dim()-3)
    Facet.set_all(0)
    Cell.set_all(0)
    cell_sub.set_all(0)
    facet_sub.set_all(0)
    ...
    for cell in df.cells(mesh) :
        for facet in df.facets(cell): 
            ...
            if(phi1(v1.point())*phi1(v2.point()) <= 0.0 or df.near(phi1(v1.point())*phi1(v2.point()),0.0) 
               or phi1(v1.point())*phi1(v3.point()) <= 0.0 or df.near(phi1(v1.point())*phi1(v3.point()),0.0) or phi1(v3.point())*phi1(v2.point()) <= 0.0 or df.near(phi1(v3.point())*phi1(v2.point()),0.0)) :  
                Cell[cell] = trou1  
                cell_sub[cell] = 1
                for facett in df.facets(cell):  
                    ...
                    for edge in df.edges(facett):
                        edge_sub[edge] = 1
                    v1, v2, v3 = df.vertices(facett)
                    vertices_sub[v1], vertices_sub[v2], vertices_sub[v3] = 1, 1, 1
    ...
    File3 = df.File("sub.rtc.xml/mesh_function_3.xml")
    File3 << cell_sub
    ...
    Z = df.TensorFunctionSpace(mesh,"CG",degV, shape = (3,3))
    Q = df.VectorFunctionSpace(mesh,"DG",degV-1, dim = 3)
    ...
    u_D = df.Constant((0.0,0.0,0.0))
    f = df.Expression(('0.0','-9.81*0.1', '0.0'), degree=4, domain = mesh)
    g = df.Constant((0.0,0.0,0.0))
    
    # Construction of the bilinear and linear forms
    boundary_penalty = sigma_p*df.avg(h)*df.inner(df.jump(sigma(u),n), df.jump(sigma(v),n))*dS(trou1) \
                     + sigma_p*h**2*(df.inner(- df.div(sigma(u)),- df.div(sigma(v))))*dx(trou1) \
                     + sigma_p*df.avg(h)*df.inner(df.jump(sigma(u),n), df.jump(sigma(v),n))*dS(trou2) \
                     + sigma_p*h**2*(df.inner(- df.div(sigma(u)),- df.div(sigma(v))))*dx(trou2) \
                     + sigma_p*df.avg(h)*df.inner(df.jump(sigma(u),n), df.jump(sigma(v),n))*dS(trou12) \
                     + sigma_p*h**2*(df.inner(- df.div(sigma(u)),- df.div(sigma(v))))*dx(trou12) 
    ...
    auv = df.inner(sigma(u), epsilon(v))*dx \
        + gamma_u*df.inner(sigma(u),sigma(v))*dx(trou1) \
        + gamma_u*df.inner(sigma(u),sigma(v))*dx(trou2) \
        + gamma_u*df.inner(sigma(u),sigma(v))*dx(trou12) \
        + boundary_penalty 

    auz = gamma_u*df.inner(sigma(u),z)*dx(trou1) \
        + gamma_u*df.inner(sigma(u),z)*dx(trou2) \
        + gamma_u*df.inner(sigma(u),z)*dx(trou12) 
    auq = 0.0
    
    ayv = df.inner(df.dot(y,n),v)*ds(trou1) + gamma_u*df.inner(y,sigma(v))*dx(trou1) \
        + df.inner(df.dot(y,n),v)*ds(trou2) + gamma_u*df.inner(y,sigma(v))*dx(trou2) \
        + df.inner(df.dot(y,n),v)*ds(trou12) + gamma_u*df.inner(y,sigma(v))*dx(trou12)
    ...
    ayz = gamma_u*df.inner(y,z)*dx(trou1) + gamma_div*df.inner(df.div(y), df.div(z))*dx(trou1) \
        ...
        + gamma_p*h**(-2)*df.inner(df.dot(y,df.grad(phi1)), df.dot(z,df.grad(phi1)))*dx(trou12)
    ayq = gamma_p*h**(-3)*df.inner(df.dot(y,df.grad(phi1)), q*phi1)*dx(trou1) \
        + gamma_p*h**(-3)*df.inner(df.dot(y,df.grad(phi2)), q*phi2)*dx(trou2) \
        + gamma_p*h**(-3)*df.inner(df.dot(y,df.grad(phi1)), q*phi1)*dx(trou12)
    
    apv = 0.0
    apz = gamma_p*h**(-3)*df.inner(p*phi1, df.dot(z,df.grad(phi1)))*dx(trou1) \
        + gamma_p*h**(-3)*df.inner(p*phi2, df.dot(z,df.grad(phi2)))*dx(trou2) \
        + gamma_p*h**(-3)*df.inner(p*phi1, df.dot(z,df.grad(phi1)))*dx(trou12)
    apq = gamma_p*h**(-4)*df.inner(p*phi1,q*phi1)*dx(trou1) \
        + gamma_p*h**(-4)*df.inner(p*phi2,q*phi2)*dx(trou2) \
        + gamma_p*h**(-4)*df.inner(p*phi1,q*phi1)*dx(trou12)
        
    lv = df.inner(f,v)*dx  \
        + sigma_p*h**2*df.inner(f, - df.div(sigma(v)))*dx(trou1) \
        + sigma_p*h**2*df.inner(f, - df.div(sigma(v)))*dx(trou2) \
        + sigma_p*h**2*df.inner(f, - df.div(sigma(v)))*dx(trou12) 
    lz = gamma_div*df.inner(f, df.div(z))*dx(trou1) + gamma_div*df.inner(f, df.div(z))*dx(trou2) \
        + gamma_div*df.inner(f, df.div(z))*dx(trou12)  
    lq = 0.0
    ...
    u_h_phi = UU[0]
    if i == end - 1 :
        plot(u_h_phi, mode = 'displacement', interactive = False, N = 2, at=0, text ='u_h phi-FEM')
    # Compute and store relative error for H1 and L2 norms
    domain = mshr.Box(df.Point(0,0,0), df.Point(1,1,1)) \
        - mshr.Sphere(df.Point(0.25,0.25,0.25), 0.15) - mshr.Sphere(df.Point(0.75,0.75,0.75), 0.15)
    H = 3*(i-1) # to have approximately the same precision as in the phi-fem computation
    ...
    if i == end - 1 :
        plot(u_h, mode = 'displacement', at = 1, text = 'u_h standard')
