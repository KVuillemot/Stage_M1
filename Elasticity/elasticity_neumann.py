...
for i in range(start,end,step): 
    ...
    auv = df.inner(sigma(u), epsilon(v))*dx + df.inner(u,v)*dx \
        + gamma_u*df.inner(sigma(u),sigma(v))*dx(1) \
        + gamma_div*df.inner(u,v)*dx(1) \
        + boundary_penalty 

    auz = gamma_u*df.inner(sigma(u),z)*dx(1) + gamma_div*df.inner(df.div(z),u)*dx(1)
    auq = 0.0
    
    ayv = df.inner(df.dot(y,n),v)*ds(1) + gamma_u*df.inner(y,sigma(v))*dx(1) + gamma_div*df.inner(df.div(y),v)*dx(1)
    ...
    lv = df.inner(f,v)*dx  \
        + gamma_div*df.inner(f,v)*dx(1)
    lz = gamma_div*df.inner(f, df.div(z))*dx(1) - gamma_p*h**(-2)*df.inner(g*phi_abs, df.dot(z,df.grad(phi)))*dx(1)
...
for i in range(start, end, step):
    ...
    g = df.dot(sigma(u_ex),df.grad(phi))/(df.inner(df.grad(phi),df.grad(phi))**0.5) \
        + u_ex*phi
    a = df.inner(sigma(u), epsilon(v))*df.dx + df.inner(u,v)*df.dx
...
plt.tight_layout()
plt.savefig('Elasticity_Neumann/relative_error_P_{name0}.png'.format(name0=degV))
plt.show()
...
plt.tight_layout()
plt.savefig('Elasticity_Neumann/Time_precision_P_{name0}.png'.format(name0=degV))
plt.show()
plt.figure()
...
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig('Elasticity_Neumann/Time_error_P_{name0}.png'.format(name0=degV))
plt.show()
plt.figure()
...
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig('Elasticity_Neumann/Total_time_error_P_{name0}.png'.format(name0=degV))
plt.show()

#  Write the output file for latex
f = open('Elasticity_Neumann/output_ghost_case1_Neumann_P{name0}.txt'.format(name0=degV),'w')
