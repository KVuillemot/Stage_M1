...
for i in range(start,end,step): 
    ...
    u_ex = df.Expression("exp(x[0])*sin(2*pi*x[1])*(-1.0/8.0 + (x[0]-0.5)*(x[0]-0.5) + (x[1]-0.5)*(x[1]-0.5))", degree=4, domain=mesh)
    ...
    # Creation of the bilinear and linear forms using stabilization terms
...
for i in range(start, end, step):
    ...
    def boundary(x,on_boundary):
        return on_boundary
    bc = df.DirichletBC(V, u_ex, boundary)
...
plt.tight_layout()
plt.savefig('Dirichlet_homogene/relative_error_P_{name0}.png'.format(name0=degV))
plt.show()
...
plt.tight_layout()
plt.savefig('Dirichlet_homogene/Time_precision_P_{name0}.png'.format(name0=degV))
plt.show()
plt.figure()
...
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig('Dirichlet_homogene/Time_error_P_{name0}.png'.format(name0=degV))
plt.show()
plt.figure()
...
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig('Dirichlet_homogene/Total_time_error_P_{name0}.png'.format(name0=degV))
plt.show()
#  Write the output file for latex
f = open('Dirichlet_homogene/output_ghost_P{name0}.txt'.format(name0=degV),'w')
